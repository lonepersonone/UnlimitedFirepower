...
namespace EpicToonFX
{
    ...
    {
        public GameObject impactParticle; // Effect spawned when projectilePrefab colliderHits a collider
        ...
        {	
            ...
            Vector3 direction = transform.GetComponent<Rigidbody>().velocity; // Gets the direction of the projectilePrefab, used for collision detection
            ...
            {
                transform.position = hit.point + (hit.normal * collideOffset); // Move projectilePrefab to point of collision
                ...
                for (int i = 1; i < trails.Length; i++) // Loop to cycle through found particle systems
                {
                    ...
                    {
                        trail.transform.SetParent(null); // Detaches the trail from the projectilePrefab
                ...
                Destroy(gameObject); // Removes the projectilePrefab
            }
        }
    }
}