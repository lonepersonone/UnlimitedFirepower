...
namespace Michsky.UI.Reach
{
    ...
    public class ShopButtonManager : MonoBehaviour, IPointerClickHandler, IPointerEnterHandler, IPointerExitHandler, ISelectHandler, IDeselectHandler, ISubmitHandler
    {
        ...
        public TextMeshProUGUI valueObj;
        ...
        public delegate bool ParchaseDelegate(UpgradeItem item);
        public event ParchaseDelegate onParchaseEvent;
        private void ParchaseCallback()
        {
            if (!onParchaseEvent.Invoke(upgradeItem))
            {
                Debug.Log("Parchase Failed");
            }
            else
            {
        ...
        public void UpdateValue(UpgradeItem item)
        {
            this.upgradeItem = item;
            this.titleLocalizationKey = $"{item.Id}Title";
            this.descriptionLocalizationKey = $"{item.Id}Desc";

            this.buttonIcon = item.Icon;
            this.priceText = item.Cost.ToString();
        ...
        {
            LocalizedObject loc = GetComponent<LocalizedObject>();
            buttonTitle = loc.GetKeyOutput(titleLocalizationKey);
            buttonDescription = loc.GetKeyOutput(descriptionLocalizationKey);
            titleObj.text = buttonTitle;
            descriptionObj.text = buttonDescription;     
        }

        public void SetValue(UpgradeItem item, ParchaseDelegate callback)
        {
            onParchaseEvent += callback;
            onPurchaseClick.AddListener(ParchaseCallback);
        ...
        public void UpdateUI()
        {
            ...
            else if (enableFilter && filterObj != null && filters.Count > 1) 
            {
                ...
                if (Application.isPlaying && backgroundFilter == BackgroundFilter.Random) { filterObj.sprite = filters[UnityEngine.Random.Range(0, filters.Count - 1)]; }
